...
左边是全书目录（可搜索）。**顶栏只留这几个**：目录 · 进度 · 日间模式 · 皮肤 · 问问题 · 写笔记。

随机抽题、错题本、清除勾选、打印/存 PDF 都挪进了目录抽屉底部的「工具」区 ——
它们不是每天要点的东西，放在那儿更清爽（手机上从「目录」进去就能看到）；
手机上用的「随机抽题 / 错题本」也可以从每章末尾的「做题」进去，那里本来就是同一个题库。

**每章末尾有三个按钮**：前往下一章、做题（只抽这一章的题）、写笔记。
读完一章顺手做完题、写下两句话，比连读三章记得牢得多。

每一章都带三维演示或交互动画，正文后面还有「动手体验 / 现实校准 / 面试怎么说」。
...
上面那张是**同一页在两套皮肤下的真实渲染**（同一个三维场景，
配色也跟着换了：经典是青灰 + 朱红，锤子是蓝 + 红）。
...
### 2026-09-25 · 重拍全部配图（终于能渲染了）

之前一直说"环境起不来浏览器、配图没法重拍"。这次把它解决了，`docs/` 里的图**全部重拍**，
现在是当前构建的真实渲染，顶栏的新按钮、章末导航、皮肤面板都在图里。

卡住的地方和绕法，记下来免得下次再踩：

1. **Chrome 在沙箱里 SIGABRT**。崩溃栈里是 `HIServices _RegisterApplication` →
   `TransformProcessType` —— 它在向 WindowServer 注册，沙箱不让。
2. 换 `chrome-headless-shell` 后又报
   `bootstrap_check_in ... MachPortRendezvousServer: Permission denied` ——
   多进程架构要做 Mach port 握手，也不让。
3. **解法是 `--single-process`**：没有子进程就不需要那个握手，页面立刻能渲染。
4. **截图必须走 CDP 的 JPEG**：CLI 的 `--screenshot` 走 PNG 编码器，
   崩溃栈里能看到是 `rust_png` 挂的；JPEG 那条路没事。

现在这套流程约 2 分钟能拍完 23 张（含 4 个手机场景、9 个三维场景单体、桌面、
两套皮肤对照、暗色、笔记与问问题面板）。
...
- `docs/` 里的图都是**当前构建的真实渲染**（2026-09-25 重拍），不是示意图、
  也没有画出来的假界面。拍图用的是 `chrome-headless-shell` + 单进程 + CDP JPEG，
  脚本在 `work/shoot_book.mjs`（配套拼图脚本 `work/make_docs_images.py`）。
